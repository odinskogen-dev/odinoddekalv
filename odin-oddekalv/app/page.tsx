    ...
    <>
      <section className="container-editorial flex min-h-[82svh] flex-col justify-center py-24 md:py-28">
        ...
        <div className="mt-10 flex flex-wrap gap-x-9 gap-y-3">
          {[["01","Read the story","/story"],["02","Read the manifest","/manifest"],["03","Current work","/projects"]].map(([i,label,href]) => (
      ...
      <section className="container-editorial mt-24 md:mt-36">
        <Reveal className="grid gap-8 border-y border-ink/10 py-12 md:grid-cols-12 md:py-16">
          <div className="md:col-span-4"><Label>Field / archive</Label></div>
          <div className="md:col-span-8"><p className="max-w-2xl text-2xl font-medium leading-snug tracking-tight text-ink md:text-3xl">The work has to remain close to what it is trying to protect.</p><p className="mt-5 max-w-xl text-lg leading-relaxed text-ink/60">Coast, weather, people, species, field work and the physical evidence behind the mission. The public archive will be built from the original image files and verified context.</p></div>
        </Reveal>
      </section>

      <section className="container-editorial mt-24 md:mt-36">
        <Reveal className="max-w-3xl border-t border-ink/10 pt-10">
          <Label>Manifest</Label>
          <h2 className="mt-5 font-medium tracking-tight text-ink [font-size:clamp(34px,5vw,68px)] [letter-spacing:-0.04em] [line-height:0.98]">ALT JEG ELSKER LEVER.</h2>
          <p className="mt-4 text-xl text-ink/65">Everything I Love Is Alive.</p>
          <p className="mt-4 max-w-xl text-ink/60">On love, loss, belonging and the decision to build for a living world.</p>
          <Link href="/manifest" className="group mt-8 inline-flex items-center gap-2 font-mono text-sm uppercase tracking-label text-ink transition-colors hover:text-blue"><span className="link-underline">Read the manifest</span><span>→</span></Link>
        </Reveal>
      </section>
