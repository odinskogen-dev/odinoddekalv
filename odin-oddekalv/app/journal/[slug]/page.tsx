  ...
  return (
    <article className="pt-16 md:pt-24">
      ...
      <div className="container-editorial mt-12">
        <Media src={post.image} alt={post.title} className="aspect-[16/9] w-full" priority />
      </div>
      ...
      <div className="container-editorial mt-14">
        <div className="mx-auto max-w-reading">
          ...
          {/* Tags */}
          <div className="mt-12 flex flex-wrap gap-2 border-t border-ink/10 pt-8">
            {post.tags.map((t) => (
              <span
                key={t}
      ...
      <nav className="container-editorial mt-20">
        <div className="grid gap-px overflow-hidden border border-ink/10 bg-ink/10 md:grid-cols-2">
          {prev ? (
            <Link href={`/journal/${prev.slug}`} className="group bg-paper p-8 transition-colors hover:bg-blue/[0.03]">
          ...
          {next ? (
            <Link
              ...
              className="group bg-paper p-8 text-right transition-colors hover:bg-blue/[0.03]"
            >
              <span className="font-mono text-xs uppercase tracking-label text-ink/40">
                Newer →
              </span>
